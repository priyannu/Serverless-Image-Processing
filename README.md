...
When an image is uploaded to an input S3 bucket, an S3 event automatically invokes an AWS Lambda function. The function processes the image using Python and Pillow, generates three resized JPEG versions and a PDF, stores the results in an output S3 bucket, and publishes a success or failure notification to Amazon SNS.

The application also supports independently configurable watermarks for the resized images and the generated PDF.
...
| Pillow | Image resizing, compression, watermarking and PDF generation |
...
| `infrastructure/setup.py` | Optional infrastructure provisioning script |
| `scripts/build_lambda.ps1` | Builds a Linux-compatible Lambda deployment package |
...
The `build/` directory and `.env` file should not be committed to GitHub.
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\build_lambda.ps1
```
...
The project supports separate watermark settings for JPEG images and PDFs.

### JPEG watermark options

A watermark can be applied to:
...
The watermark is rendered directly onto the selected image outputs.

### PDF watermark

The PDF has independent watermark settings.

For example:

```text
Small JPG   : No watermark
Medium JPG  : PROJECT DEMO
Large JPG   : PROJECT DEMO
PDF         : CONFIDENTIAL
```
...
```json
{
  ...
  "image_watermark_text": "PROJECT DEMO",
...
**Current implementation note:** An automatic S3 event contains object information, not arbitrary interactive watermark settings. The Lambda handler can accept the configuration above during a direct invocation, but the S3-triggered path currently uses default settings unless an additional configuration mechanism is implemented.

A future upload interface can save settings in S3 object metadata or a companion JSON object so that each upload has its own watermark configuration.
...
Unsupported, empty or corrupted files should be rejected and reported as processing failures.
...
This is an illustrative example. Actual dimensions and log output depend on the uploaded image and implementation.
...
- [ ] Support per-image watermark settings through S3 metadata or companion JSON.
...
GitHub: [Shubhambehera29](https://github.com/Shubhambehera29)
